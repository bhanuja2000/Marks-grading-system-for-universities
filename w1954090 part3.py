    ...
    while True:
        ...
        if target == 'y':
            ...
            for credit in progress_count_list:
                ...
                x.write('progress-'+str(credit)[1:len(str(credit))-1]+'\n')
            
            for credit in Progress_module_trailer_list:
                
                x.write('Trailer-'+str(credit)[1:len(str(credit))-1]+'\n')
            
            for credit in Do_not_Progress_list:
                
                x.write('Do not Progress-'+str(credit)[1:len(str(credit))-1]+'\n')
            
            for credit in exculdc_list:
                
                x.write('Exclude-'+str(credit)[1:len(str(credit))-1]+'\n')
            x.close()
            f=open('h1.txt','r')
            d=f.read()
            print(d)
